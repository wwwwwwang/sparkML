...
object Regressor {
  ...
  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("decision_tree regressor")
    ...
    for (depth <- depths; bin <- bins) {
      val model = DecisionTree.trainRegressor(trainingData.map(_._2), categoricalFeaturesInfo,
      ...
      if (ratio < 1.0) {
        ...
        val alg = s"number: $cnt, categoricalFeaturesInfo: ${categoricalFeaturesInfo.toString()}," +
          s"impurity: $impurity, maxDepth: $depth, maxBins: $bin, mse: $MSE"
    ...
    if (1 != cnt) {
      val msg = "In decision_tree regressor job " + jobID + ", the best result is got by " +
