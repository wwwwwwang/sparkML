...
object K_Means {
  ...
  def main(args: Array[String]) {
    ...
    if (configs.containsKey("epsilon")) {
      epsilon = configs.get("epsilon").toDouble
    }
    if (configs.containsKey("runs")) {
      runs = configs.get("runs").toInt
    }
    if (configs.containsKey("initializationmode")) {
      initializationMode = configs.get("initializationmode")
    }
    ...
    log.info(s"#####################k = $k")
    log.info(s"#####################initializationMode = $initializationMode")
    log.info(s"#####################epsilon = $epsilon")
    ...
    val ks = k.split(",")
    for (k <- ks) {
      val KMalg = new KMeans().setEpsilon(epsilon).setK(k.toInt)
        .setMaxIterations(maxIterations).setRuns(runs)
        .setInitializationMode(initializationMode)
      val model = KMalg.run(parsedData.map(_._3))
      //val model = KMeans.train(parsedData, k, maxIterations, runs,
      ...
      if (hasLabel) {
        ...
        val alg = s"number: $cnt, k: $k, maxIterations: $maxIterations, runs: $runs, " +
          s"initializationMode: $initializationMode, epsilon: $epsilon, F_measure: $F_measure"
        ...
        val alg = s"number: $cnt, k: $k, maxIterations: $maxIterations, runs: $runs, " +
          s"initializationMode: $initializationMode, epsilon: $epsilon, MSE: $WSSSE"
        val oneTrain = TrainResult(jobID, alg, timestamp)
        res = res.::(oneTrain)
      }
    ...
    log.info(s"train_information had been saved into mysql table train_information")
