...
object Regressor {
  ...
  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("gradient_boosted_tree regressor")
    ...
    for (depth <- depths; lr <- learnRates) {
      ...
      if (ratio < 1.0) {
        ...
        log.info("#####################Learned regression GBT model:\n" + model.toDebugString)
        if (MSE < mse) {
          best = cnt
          mse = MSE
        }
    ...
    if (cnt != 1) {
      val msg = "In gradient_boosted_tree regressor job " + jobID + ", the best result is got by " +
