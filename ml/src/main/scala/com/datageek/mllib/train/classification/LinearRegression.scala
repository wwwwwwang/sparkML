...
object LinearRegression {
  val log = LogFactory.getLog(LinearRegression.getClass)
  ...
  def main(args: Array[String]) {
    ...
    for (st <- steps; reg <- regParams) {
      val LRalg = new LinearRegressionWithSGD().setIntercept(intercept)
      ...
      if (ratio < 1.0) {
        ...
        val alg = s"number: $cnt, iterations: $iterations, step: $st, regParam: $reg, " +
          ...
          s"convergenceTol: $convergenceTol, mse: $MSE"
    ...
    if (cnt != 1) {
      val msg = "In linear_regression job " + jobID + ", the best result is got by " +
